...
export default async function KidsDetailPage({
  params,
}: {
  params: { locale: string; slug: string };
}) {
  ...
  const project = await getProjectBySlug(params.slug);
  if (!project || project.category !== "KIDS" || !project.published) notFound();

  return (
    <Section tone="cream">
      <Container className="max-w-3xl">
        <LinkButton href={`/${locale}/kids`} variant="ghost" size="sm" className="mb-6">
          ← {dict.cta.back}
        </LinkButton>
